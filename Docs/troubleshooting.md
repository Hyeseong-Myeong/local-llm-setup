...
## 16. 🔒 [2026-08-21] ChromaDB pre-auth RCE(CVE-2026-45829) 노출 평가 및 네트워크 격리
...
* 공격 경로: `/api/v2/tenants/{tenant}/databases/{db}/collections`에 악성 model repository와 `trust_remote_code=true`를 전달.
...
* 코드 4곳 모두 인증 인자 없이 접속 — `wiki_agent.py:159`, `fastapi_wiki_server.py:36`, `reembed_chroma.py:16`, `recreate_db.py:13`.
* **Chroma의 token authn을 켜도 이 CVE는 막히지 않는다**(pre-auth). 따라서 실질 방어선은 **네트워크 계층 하나뿐**이다.

### 🟢 노출 경계 — 실측 결과는 제한적이었다
| 경로 | 결과 | 확인 방법 |
|---|---|---|
| 인터넷 | **미노출** | NAS 공인 IP로 TCP 8000 연결 실패 |
| 개발 PC의 가정 LAN | **해당 없음** | ARP 이웃에 Synology(MAC `00-11-32*`) 없음. NAS는 이 회선에 없고 Tailscale이 공인 IP로 direct 연결 중 |
| tailnet | **도달 가능** | 단일 계정(`dev.hyeseong@`) 소유 기기 5대, 외부 공유 사용자 없음 |
| NAS가 속한 원격지 LAN | **미확인** | 개발 PC에서 확인 불가. Docker 퍼블리싱이 `0.0.0.0`이면 그쪽 회선 전체에 열려 있음 |

* **잔존 위험:** 기기 5대 중 **한 대만 침해돼도 NAS에서 무인증 RCE**. 특히 `s23`은 39일째 offline이라 패치 상태를 알 수 없는 약한 고리다.

### 🟢 [2026-08-21] 결정: Tailscale ACL + 특정 IP 바인딩
...
```json
{
  "hosts": {
    ...
    "desktop": "<개발 PC Tailscale IP>",
    "macbook": "<맥북 Tailscale IP>",
    "iphone":  "<아이폰 Tailscale IP>",
    "android": "<안드로이드 Tailscale IP>"
  },
  ...
  "acls": [
    ...
    // 3) NAS 이외 기기 간 통신은 기존 allow-all 유지
    { "action": "accept", "src": ["*"], "dst": ["desktop:*", "macbook:*", "iphone:*", "android:*"] }
  ],
...
```
* **기본 규칙을 반드시 제거해야 한다.** 스타터 정책의 `{"action":"accept","src":["*"],"dst":["*:*"]}`를 남겨두면 위 1·2번이 전부 무의미해진다.
* Tailscale ACL은 **deny 규칙이 없는 default-deny 모델**이라, "8000만 차단"은 위처럼 **포트 범위를 쪼개서** 표현해야 한다.
* `tests` 블록은 적용 전에 관리 콘솔이 자동 검증해준다. **"Preview rule changes"로 먼저 확인**할 것.
* **기기를 추가하면** `hosts`와 3번 규칙에 함께 등록해야 한다. 빠뜨리면 그 기기만 조용히 통신이 끊긴다.

#### 2) NAS docker-compose — `0.0.0.0` 바인딩 제거
```yaml
services:
  chromadb:
    ports:
      - "127.0.0.1:8000:8000"           # NAS 로컬
      - "${CHROMA_BIND_IP}:8000:8000"   # Tailscale 인터페이스 (.env에 실제 IP)
```
* 원격지 LAN 노출(위 표의 "미확인" 항목)을 원천 차단한다. 섹션 14의 Bifrost 이중 바인딩과 같은 형태.
* **주의 — 섹션 14의 교훈이 그대로 적용된다:** Tailscale 인터페이스가 컨테이너 시작보다 늦게 올라오면 바인딩이 실패하는데, `restart: unless-stopped`가 그 에러를 삼킨다. 재부팅 후 반드시 아래로 검증할 것.
```bash
# healthy 여부가 아니라 실제 퍼블리싱을 봐야 한다 (섹션 14 교훈)
docker inspect chromadb --format '{{json .NetworkSettings.Ports}}'   # 비어 있으면 바인딩 실패
```
* 검증: 개발 PC에서 `200`, 다른 tailnet 기기에서 타임아웃이면 정상.
...
* **pre-auth 취약점에 인증 추가는 완화책이 아니다.** 인증 이전 단계가 뚫리는 것이므로 네트워크 도달성을 줄이는 것 외에 방법이 없다.
* tailnet은 "사설망이라 안전"이 아니라 **"기기 하나가 곧 침해 경로"**다. 기기 수가 곧 공격 표면이며, 오래 offline인 기기가 가장 약한 고리다.

---

## 17. ⚙️ [2026-08-21] `benchmark.yml` 분리 검증 중 발견한 결함 2건
...
### 🔴 결함 2: `max_tokens`가 적용되지 않음 — Bifrost가 Ollama로 변환하지 않는다 (원인 규명, 미조치)
...
* **미조치:** 수정 방향(토큰/초 같은 정규화 지표로 전환, thinking 비활성화 전달, 또는 Bifrost 측 변환 확인)은 별도 결정 사항으로 남긴다.
