...
**소유자 확정 — `winnat` 서비스다.** `net stop winnat` / `net start winnat` 전후로 예약 구간을 비교했다.

| 시점 | TCP 예약 구간 |
|---|---|
| 전 | `5357`, `7435-8236`(100단위 7블록), `8564-8663`, `7895`, `7936`, `50000-50059 *` |
| `net stop winnat` 후 | `5357`, `7895`, `7936`, `50000-50059 *` |
| `net start winnat` 후 | `5357`, `7895`, `7936`, `50000-50059 *` |

* **100포트 블록은 전부 WinNAT 가 잡은 것이다.** 서비스를 멈추자 통째로 사라졌다. 남은 넷은 WinNAT 와 무관한 별개 예약이다.
* **서비스를 다시 켜도 곧바로 돌아오지 않았다.** 예약은 winnat 기동 시점이 아니라 **다른 계기**(부팅, 또는 NAT 네트워크 생성 시점)에 잡힌다.
* ⚠️ **"지금 비어 있다"를 근거로 포트를 고르면 안 된다.** 현재 8080 을 포함해 8000번대가 비어 있으나, 애초에 그 장애가 부팅 후에 나타났다. **예약 대상이 되지 않는 대역을 쓰는 것이 유일하게 안정적이다.**
* **미규명으로 남기는 것:** 100포트 블록의 **시작 위치를 무엇이 정하는가.** 소유자가 확정돼 실용적 대응(대역 회피)이 가능하므로 더 파지 않는다.
...
* 따라서 원인은 Ollama가 아니라 **게이트웨이 안**이다.
* ⚠️ 조사 중 `bifrost.yaml`에 파라미터 매핑 설정이 없다는 점을 근거로 들 뻔했으나, **이 배포는 그 파일을 마운트하지 않는다.** 설정은 `data/config.db`(SQLite configstore)에서 온다 — 저장소에 파일이 있다는 사실이 그 파일이 쓰인다는 뜻은 아니다. 결론은 실측으로 독립 확인됐으므로 유지하되, 근거로는 쓰지 않는다.
* **버전:** 이 환경은 `maximhq/bifrost` **v1.6.4**(이미지 빌드 2026-07-14). 공식 문서는 `max_completion_tokens`를 지원 파라미터로 표기하고 *"최소 16으로 강제된다"* 고 적고 있으나, **16과 32 모두 무시됐다.**

#### 상류 이슈로 원인 확정 — 버려지는 것이 아니라 **이름이 바뀐다**

동일 증상이 [maximhq/bifrost#6132](https://github.com/maximhq/bifrost/issues/6132) 에 보고돼 있다(2026-08-13 등록, **OPEN**). 다른 모델(`qwen2.5vl:32b`, `qwen3:30b`, `gpt-oss:120b`)과 다른 배포 형태에서도 같은 증상이며, **우리 환경 고유 문제가 아니라 Ollama 프로바이더의 결함**이다.

이슈에 올라온 소스 추적 요지 — *(코드 경로는 이슈 인용이며 이 저장소에서 직접 확인한 것은 아니다.)*

1. 인그레스에서 클라이언트의 `max_tokens`를 `ChatParameters.MaxCompletionTokens`로 접어 넣는다.
2. 이그레스 분기에서 **Mistral 계열만** 값을 `max_tokens`로 되돌린다. **Ollama는 케이스가 없어 기본 경로로 떨어진다.**
3. 결과적으로 Bifrost는 Ollama의 OpenAI 호환 엔드포인트로 **`max_completion_tokens`라는 이름으로** 보낸다.

**마지막 고리는 이 환경에서 직접 확인했다.**

| Ollama `/v1/chat/completions` 직접 | 결과 |
|---|---|
| `max_tokens=16` | `16`, `finish_reason=length` |
| `max_completion_tokens=16` | **`171`, `stop` — 무시된다** |
| 둘 다 전송 | `16`, `length` (`max_tokens`가 이긴다) |

연쇄가 완결된다. 클라이언트가 무엇을 보내든 게이트웨이가 한 이름으로 접어 넣고, Ollama의 OpenAI 호환 표면은 그 이름을 보지 않는다. **`stop`이 통과한 이유도 같은 원리로 설명된다 — 이름이 바뀌지 않는 파라미터이기 때문이다.**

* **버전과 무관한 결함이다.** 상류 보고에서 서로 다른 세 빌드(2026-07-24 · 2026-08-12 · v1.6.11) 모두 같은 동작이 확인됐다. **따라서 버전 업그레이드는 대응 선택지에서 제외한다.**
* 🔴 **thinking 비활성화 경로도 막혀 있다.** `think: false`는 네이티브 `/api/chat` 필드라 OpenAI 호환 표면이 무시하고, `reasoning_effort: "none"`은 reasoning 필드만 없앨 뿐 생성량은 거의 그대로였다(이슈 측정). Ollama 프로바이더가 모든 작업에 `/v1/*`를 쓰므로 네이티브 필드에 도달할 방법이 없다.
* 따라서 **게이트웨이를 경유하는 한 상한도 thinking 차단도 불가능**하며, 아래 토큰/초 정규화가 사실상 유일하게 남은 완화책이다.
* 이슈는 부수 영향도 지적한다 — **클라이언트가 연결을 끊어도 상류 생성이 취소되지 않는다.** 폭주한 호출을 클라이언트 쪽에서 멈출 수 없다는 뜻이라, VRAM이 빠듯한 환경에서는 상한 문제보다 더 아플 수 있다.
* **이슈를 구독하고, 수정이 들어오면 재측정한다.**
